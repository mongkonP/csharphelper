...
using System.Linq;
  ...
  namespace  howto_crypto_notepad
...
static class CryptoStuff
    {
        ...
        public static byte[] CryptBytes(string password, byte[] in_bytes, bool encrypt)
        {
            ...
            byte[] key = null;
            byte[] iv = null;
            byte[] salt = { 0xA0, 0x0C, 0x71, 0x23, 0x75, 0xBA, 0x95, 0x6D, 0x1F, 0xF0, 0xEE, 0x21, 0x82, 0x46 };
            MakeKeyAndIV(password, salt, key_size_bits, block_size_bits, out key, out iv);
        ...
        {
#if LINQ_VERSION
            // Split, iterate, and convert in one step.
            var query =
                from value in hex.Split(' ', '-')
                select Convert.ToByte(value, 16);
